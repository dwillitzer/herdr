...
/// Install official Grok Build integration under `~/.grok/hooks/`.
///
/// Grok discovers user hooks from `~/.grok/hooks/*.json` (Claude-compatible
/// nested groups). Herdr owns a dedicated managed file so install/uninstall
/// never rewrites unrelated user hook files.
pub(crate) fn install_grok() -> io::Result<GrokInstallPaths> {
    let dir = grok_dir()?;
    if !dir.is_dir() {
        return Err(io::Error::other(format!(
            "grok config directory not found at {}. install Grok Build (x.ai grok CLI) first",
            dir.display()
        )));
    }
    ...
    let mut hooks_file = json!({ "hooks": {} });
    let hooks = ensure_hooks_object(
        &mut hooks_file,
        &hooks_path,
        "grok hooks file",
        "grok hooks file hooks",
    )?;
    // Lifecycle events reject matchers in Grok Build; omit matcher for all.
